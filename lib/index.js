...
exports.deploy = async function ({ src, target, branch, remote, template, title, dryRun, history, betterTarget }) {
  ...
  const compDirectoryPath = path.resolve(process.cwd(), 'comp');
  await ncp(path.resolve(compDirectoryPath, src), path.resolve(tmpDir, target));
 ...
 .filter(dir => dir.indexOf('.') !== 0 && dir !== 'index.html' && dir !== 'react' && dir !== 'html' && dir !== 'js-modules')
 .sort((a, b) => {
    // Check if either directory is "latest" and prioritize it
    if (a === 'latest') return -1;
    if (b === 'latest') return 1;

    // Convert directory names to version numbers for comparison
    const versionA = semver.coerce(a);
    const versionB = semver.coerce(b);

    // If both versions are valid, compare them
    if (versionA && versionB) {
      return semver.rcompare(versionA, versionB);
    }
    // If only one version is valid, it should come first
    else if (versionA) {
      return -1;
    } else if (versionB) {
      return 1;
    }
    // If both are not valid, compare as strings
    else {
